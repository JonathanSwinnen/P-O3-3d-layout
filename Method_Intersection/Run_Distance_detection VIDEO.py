...
u = 0 * np.ones((3, 1))
stac = 1
stdm = np.array([[0.1],[0.2],[1]])

tracker = Tracker(u, stac, stdm, 0.1)

tracker.add_person("Persoon1", np.array([[5], [1], [5], [0], [0], [0]]))
tracker.add_person("Persoon2", np.array([[6], [1], [5], [0], [0], [0]]))
    ...
    if success:
        ...
        if key % 256 == 27:
            ...
            for pers in tracked_points:
                ...
                cv2.rectangle(frame_1, (a, b), (a + c, b + d), (255, 0, 210), 5)

                # print on image 2(point_on_img)
                a, b, c, d = int(point_on_img[1][0]), int(point_on_img[1][1]), 20, 20

                cv2.rectangle(frame_2, (a, b), (a + c, b + d), (255, 0, 210), 5)
            ...
            # Plot dets
            for det in dets:
                print(np.array([det]).T)
                point_on_img = positioner.reprojectPoint(np.array([det]).T)
                a, b, c, d = int(point_on_img[0][0]), int(point_on_img[0][1]), 5, 5
                cv2.rectangle(frame_1, (a, b), (a + c, b + d), (255, 150, 100), 5)
                a, b, c, d = int(point_on_img[1][0]), int(point_on_img[1][1]), 5, 5
